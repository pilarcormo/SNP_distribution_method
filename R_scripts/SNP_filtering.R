...
g <- g +theme_bw()
g <- g + theme_minimal(base_size = 15) +  scale_color_manual(values=Palette2, name  ="Back-cross") + scale_shape_discrete(name  ="Back-cross")+  theme(legend.position="none")
...
t <- t + theme_bw() 
t <- t + theme_minimal(base_size = 15) +  scale_color_manual(values=Palette2, name  ="Back-cross") + scale_shape_discrete(name  ="Back-cross") 
...
s <- s +theme_bw() 
s <- s+ theme_minimal(base_size = 15) +  scale_color_manual(values=Palette, name  ="Out-cross")+ scale_shape_discrete(name  ="Out-cross")
...
h <- h + theme_bw()
h <- h+ theme_minimal(base_size = 15) + scale_color_manual(values=Palette, name  ="Out-cross")+ scale_shape_discrete(name  ="Out-cross") + theme(legend.position="none")
