


hm1 <- read.table("~/SNP_distribution_method/Aw_sup1-2/Variant_calling/sup1_2_1/hm.txt", quote="\"")
ht1 <- read.table("~/SNP_distribution_method/Aw_sup1-2/Variant_calling/sup1_2_1/ht.txt", quote="\"")
hm4 <- read.table("~/SNP_distribution_method/Aw_sup1-2/Variant_calling/sup2_2_4/hm.txt", quote="\"")
ht4 <- read.table("~/SNP_distribution_method/Aw_sup1-2/Variant_calling/sup2_2_4/ht.txt", quote="\"")

hm <- read.table("~/SNP_distribution_method/Reads/OCF2/Interesting_2/hm_nocen.txt", quote="\"")
ht <- read.table("~/SNP_distribution_method/Reads/OCF2/Interesting_2/ht_nocen.txt", quote="\"")
#ratio <- read.table("~/SNP_distribution_method/Reads/m_mutants/C_chromosome5/interesting_5/ratios_BC.txt", quote="\"")
ratio <- read.table("~/SNP_distribution_method/arabidopsis_datasets/No_centromere/bcf2_nocen_chr3_100kb/Ratio_0_1/ratios.txt", quote="\"")







data_hm <- c(hm$V1)      
data_ht <- c(ht$V1)
ratio <- c(ratio$V1)
length <- 25000000
 
d1 <-density(data_hm, adjust = 1, kernel = c("gaussian"))
d2 <- density(data_ht, adjust = 3, kernel = c("gaussian"))
#d3 <- density(ratio, adjust = 1, kernel = c("gaussian"))
p1 <- plot(range(d1$x, d2$x), range(d1$y, d2$y), type = "n", main = "  ", xlim =c(0,length), xlab = " ", ylab = " ")
lines(d1, col = "magenta2") ##Homozygous 
lines(d2, col = "royalblue2", lty=2)
##lines(d3, col = "gray46", lwd =5)

legend("topright",col=c("magenta2", "royalblue2", "grey46"),lwd=1,lty=1:2,
       legend=c("Homozygous SNP density","Heterozygous SNP density"), bty="n")



